...
function KV:Dump(name, tbl)
	return table.concat({'"', name, '" ', serialize_block(tbl)}, "") -- returns the stringified table
end
function KV:Save(file, tbl)
	local str = self:Dump(file, tbl)
	return StringToFile(file, str)
end
function KV:Load(file)
	local str = FileToString(file)
	return self:Parse(str)[file]
end
